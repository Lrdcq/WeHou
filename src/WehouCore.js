...
WEHOUCORE.random=function(){
	...
	var tp=tp%1000/1000;
	WEHOUCORE.randomuse++;
	if(WEHOUCORE.randomuse>1000){WEHOUCORE.randomuse=0;}
	return tp;
}
					...
					if(g.ms.hit<=0){
						...
						if(ns.enditem){
							for(var cc=0;cc<ns.enditem.length;cc++){
								WEHOUCORE.nowitem.push([[ns.enditem[cc][0],g.ms.position.x,g.ms.position.y,ns.enditem[cc][1],ns.enditem[cc][2]]]);
							}
						}
			...
			if(this.type=='enemy'){
				if(tz.ms.hit<0){
					...
					if(ls=this.item.length){
						for(var c=0;c<ls;c++){
							WEHOUCORE.nowitem.push([[this.item[c][0],tz.ms.position.x,tz.ms.position.y,this.item[c][1],this.item[c][2]]]);
						}
					}
				}
			}

...
WEHOUGAME.intGame=function(dom){
	...
	var rttshader=new THREE.ShaderMaterial({
		...
		'	}'+
		'	if(is>0.0&&dis<80.0){'+
		'		fc.x=fc.x+(1.0-fc.x)*(80.0-dis)/600.0;'+
		'	}'+
		'	gl_FragColor = fc;'+
		'}'
	});
	...
	WEHOUCORE.deal.cheathit=WEHOUCORE.deal.hit=function(){//处理撞击
		if(!WEHOUGAME.misscd){
			...
			WEHOUGAME.misscd=60;
			WEHOUGAME.miss++;
			WEHOUGAME.basescore=Math.floor(WEHOUGAME.basescore*0.8);
		}
	}
...
WEHOUGAME.gamestart=function(){
	...
	//
	if(window.localStorage&&localStorage.d7_hs){
		WEHOUGAME.hiscore=localStorage.d7_hs;
	}//
...
WEHOUGAME.gameover=function(){
	...
	//localStorage
	if(window.localStorage){
		localStorage.d7_hs=WEHOUGAME.hiscore;
	}
	//
	WEHOUGAME.tempmesh[4].material.map.needsUpdate = true;
	WEHOUGAME.ui.scene.add(WEHOUGAME.tempmesh[4]);
...
WEHOUGAME.render=function() {
	...
	if(WEHOUCORE.time%WEHOUGAME.slowtime==0){
		...
		if(WEHOUGAME.kd[4]){
			mv=2.4;WEHOUCORE.nowself.useGroup=1;
		}else{
			mv=5;WEHOUCORE.nowself.useGroup=0;
		}
		...
		if(WEHOUGAME.score>WEHOUGAME.hiscore)WEHOUGAME.hiscore=WEHOUGAME.score;
		document.getElementById('score').innerHTML=WEHOUGAME.score;
		document.getElementById('hiscore').innerHTML=WEHOUGAME.hiscore;
		document.getElementById('basescore').innerHTML=WEHOUGAME.basescore;

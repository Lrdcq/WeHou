...
WEHOUGAME.addBoss=function(img,w,h,cut){
	...
	WEHOUGAME.myplalistbos.l[n].add(WEHOUGAME.myplalist.l[4].clone());
	WEHOUGAME.myplalistbos.l[n].add(WEHOUGAME.myplalist.l[5].clone());
	WEHOUGAME.myplalistbos.l[n].act=function(t){
	...
	WEHOUGAME.myplalistbos.l[n].sp=function(t){
		var sl=Math.sin(WEHOUCORE.time/50)*0.4;
		var mc=this.children[0];
		mc.rotation.z+=0.1;
		mc.scale.x=2.35+sl;
	...
	WEHOUGAME.myplalistbos.l[n].end=function(t){
		var mc=this.children[0];
		mc.rotation.z+=0.1;
		mc.scale.x+=0.05;
		mc.scale.y+=0.05;
...
WEHOUGAME.intGame=function(dom){
	...
	WEHOUGAME.bg.renderer.setClearColorHex(0x08142a, 1.0);
	...
	//加载SE
	WEHOUGAME.addSe(['se/se_tan01.ogg','se/se_kira00.ogg','se/se_boon01.ogg','se/se_lazer01.ogg','se/se_timeout2.ogg','se/se_pldead00.ogg','se/se_cat00.ogg','se/se_enep01.ogg']);
	//子弹击中 子弹散开 开扇 开激光 时间到了 MISS SC展开 爆炸
	//WEHOUGAME.playSe(6);
	//SP背景
	WEHOUGAME.tempspbg[0]=THREE.ImageUtils.loadTexture('img/sp_1_bg.jpg');
	WEHOUGAME.tempspbg[1]=THREE.ImageUtils.loadTexture('img/sp_1_fl.png');
	WEHOUGAME.tempspbg[2]=THREE.ImageUtils.loadTexture('img/sp_2_bg.jpg');
	WEHOUGAME.tempspbg[3]=THREE.ImageUtils.loadTexture('img/sp_2_fl.png');
	...
	cxt.lineWidth = 1;
	//内置特效
	var eff_1=WEHOUGAME.addBullet(WEHOUGAME.getBulletImg(2),30,30,0,'ball',1,[3/8,4/8,7/8,6/8]);
	var eff_2=WEHOUGAME.addBullet(WEHOUGAME.getBulletImg(2),128,128,0,'ball',1,[8/16,9/16,15/16,1]);
	WEHOUGAME.mainstory.effect.add([
		{mesh:WEHOUGAME.getBullet(eff_1),period:50,position:vec2(0,0),positionrandom:vec2(0,0),lifetime:100,valpha:-0.01,size:vec2(0,0),vsize:vec2(0.08,0.08),asize:-0.0008,anglerandom:360},
		{mesh:WEHOUGAME.getBullet(eff_2),period:50,position:vec2(0,0),positionrandom:vec2(50,50),lifetime:100,valpha:-0.01,size:vec2(0,0),vsize:vec2(0.1,0.1),asize:-0.0005,anglerandom:360}
	]);
	//
		...
		if(t==0){
			...
			if(WEHOUGAME.topscore>WEHOUGAME.hiscore){
				...
				if(window.localStorage){
					localStorage.d5_hs=WEHOUGAME.hiscore;
					WEHOUGAME.hiscore=localStorage.d5_hs;
				}
			}
			//
			WEHOUGAME.miss=0;
	...
	WEHOUGAME.mainstory.actlist[14]=function(msg,t,nt){
		document.getElementById('topscore').innerHTML=WEHOUGAME.mainstory.l[0].bl.l[0].ballbox.length;
	}
	//
...
WEHOUGAME.gamestart=function(){
	...
	//
	if(window.localStorage&&localStorage.d5_hs){
		WEHOUGAME.hiscore=localStorage.d5_hs;
	}//
...
WEHOUGAME.render=function() {
	...
	if(WEHOUCORE.time%WEHOUGAME.slowtime==0){
		...
		WEHOUGAME.bg.callback(WEHOUCORE.time);
	}
	//渲染
